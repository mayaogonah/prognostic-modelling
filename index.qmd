...
tbi_descriptive <- prognosis3 %>%
  mutate(sex = ifelse(sex == 0, 'female', 'male'),
         'history of suicidality' = ifelse(pretbi_suicidality == 1, 'yes', 'no'),
...
```{r}
full_model <- coxph(Surv(time, endpoint) ~ sex + rcs(age_at_entry, 3) + deprivation + pretbi_suicidality + pretbi_suicidality + pretbi_conduct_disorder + pretbi_neurodevelopmental + pretbi_learning_difficulties + pretbi_psychiatric + pretbi_neurological + pretbi_sud + tbi_severity_binary, data = prognosis3, x = T, y = T)
...
int_val <- bootstrap_cv(db = prognosis3,
                        ...
                        time = "time", 
                        status = "endpoint", 
                        formula_model = "Surv(time, endpoint) ~ sex + rcs(age_at_entry, 3) + deprivation + pretbi_suicidality + pretbi_suicidality + pretbi_conduct_disorder + pretbi_neurodevelopmental + pretbi_learning_difficulties + pretbi_psychiatric + pretbi_neurological + pretbi_sud + tbi_severity_binary",
