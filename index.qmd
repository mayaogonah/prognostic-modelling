---
title: "Predicting Violent Victimisation in Adolescent TBI Patients: R code"
subtitle: "For this project, I developed clinical prediction models to estimate the risk of victimisation following adolescent traumatic brain injury (TBI). This document outlines the key steps in the model development and internal validation, and has been prepared in accordance with [SAIL Databank's](https://saildatabank.com) output review policy. This document is addresses three goals:<br> 
...
Information on the outcome of interest is included at this stage. Variable `victimisation_date` contains the date of a diagnosis (defined using ICD-10 or Read Codes) of an injury purposely inflicted by another. Follow-up now needs to finish on the date of violent victimisation, for those with this outcome.
...
Create a table with all violent victimisation events.
...
Outcome events within 7 days of the index TBI were excluded in order to account for the acute post-injury phase, and reduce risk of reverse causation. Individuals with outcome events within the first 7 days of follow-up remained at risk, and subsequent violent victimisation events (after the removed incidence) were included.

```{r, echo=FALSE}
# add the next victimisation event during follow-up
test <- temp %>%
  filter(is.na(victimisation_date) & !is.na(second_victimisation_date)) %>%
  dplyr::select(alf, dod, entry_date, end_date, tbi_date, tbi_life, victimisation_date, second_victimisation_date)
```
...
# add the next victimisation event during follow-up
temp <- temp %>%
  mutate(victimisation_date = if_else(
    (between(as.numeric(difftime(victimisation_date, tbi_date, units = "days")), 0, 7)), NA, as.Date(victimisation_date))
  )

temp <- temp %>%
  mutate(victimisation_date = case_when(is.na(victimisation_date) & (as.numeric(difftime(second_victimisation_date, tbi_date, units = "days")) > 7) ~ as.Date(second_victimisation_date), TRUE ~ victimisation_date))
...
```{r, echo=FALSE}
temp3 <- temp2 %>%
  dplyr::select(-victimisation_life, -victimisation_date, -suicidality_life, -suicidality_date, sud_life, -sud_date, -psychiatric_life, -psychiatric_date, -adhd_life, -adhd_date, -autism_life, -autism_date, -conduct_disorder_life, -conduct_disorder_date, -learning_difficulties_life, -learning_difficulties_date, -neurological_life, -neurological_date, -child_maltreatment_flag, -looked_after_life, -looked_after_date)
...
```{r}
full_model <- coxph(Surv(time, endpoint) ~ sex + rcs(age_at_entry, 3) + deprivation + pretbi_victimisation + pretbi_victimisation + pretbi_conduct_disorder + pretbi_neurodevelopmental + pretbi_learning_difficulties + pretbi_psychiatric + pretbi_neurological + pretbi_sud + tbi_severity_binary, data = prognosis3, x = T, y = T)
